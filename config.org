...
Ensure GUI Emacs and daemon mode inherit PATH and other env vars from shell
#+begin_src emacs-lisp
...
;; keywords
(setq org-todo-keywords '((sequence "TODO(t)" "IN-PROGRESS(i)" "PAUSED(p)" "WAITING(w)" "BLOCKED(b)" "FINALIZE(f)" "|" "DONE(d)" "MISSED(m)" "CANCELLED(c)")
...
(use-package magit
  ...
  (setq magit-process-connection-type 'pty)
...
(message "rust analyzer: %s" (executable-find "rust-analyzer"))
...
(use-package rustic
  ...
  (setq lsp-rust-analyzer-rustfmt-enable t)
  :hook (rustic-mode . (lambda ()
                         (lsp-deferred)  ;; Just this, nothing else
